...
function(define_component_executable COMP_NAME)
	string(TOUPPER ${COMP_NAME} COMP_NAME_UPPER)
	set(${COMP_NAME_UPPER}_BUILD_EXE 1 PARENT_SCOPE)
	component_helper()
endfunction()

function(define_component_library COMP_NAME BUILD_STATIC BUILD_SHARED)
	string(TOUPPER ${COMP_NAME} COMP_NAME_UPPER)
	if(BUILD_STATIC)
...
function(add_component_dependencies COMP_NAME COMP_DEPS)
	...
	foreach(COMP_DEP_NAME ${COMP_DEPS})
		...
		if(${COMP_NAME_UPPER}_BUILD_EXE)
			...
			if(${COMP_NAME_UPPER}_BUILD_STATIC)
				if(${COMP_DEP_NAME_UPPER}_BUILD_STATIC)
					target_link_libraries(${COMP_NAME}_Lib LINK_PUBLIC ${COMP_DEP_NAME}_Lib)
				elseif(${COMP_DEP_NAME_UPPER}_BUILD_SHARED)
					target_link_libraries(${COMP_NAME}_Lib LINK_PRIVATE ${COMP_DEP_NAME}_Shared)
				endif()
			endif()
			if(${COMP_NAME_UPPER}_BUILD_SHARED)
				if(${COMP_DEP_NAME_UPPER}_BUILD_SHARED)
					target_link_libraries(${COMP_NAME}_Shared LINK_PUBLIC ${COMP_DEP_NAME}_Shared)
				elseif(${COMP_DEP_NAME_UPPER}_BUILD_STATIC)
					target_link_libraries(${COMP_NAME}_Shared LINK_PRIVATE ${COMP_DEP_NAME}_Lib)
				endif()
			endif()
		endif()
	endforeach()
